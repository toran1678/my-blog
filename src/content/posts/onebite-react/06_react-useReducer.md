...
## Context의 한계와 컴포넌트 합성

React Context는 Props Drilling을 해결해주는 편리한 도구이지만, 남용할 경우 다음과 같은 문제점이 발생할 수 있습니다.

### 1. Context의 문제점
- **컴포넌트 재사용성 저하**: `useContext`를 사용하는 컴포넌트는 특정 Provider의 하위에서만 동작하게 됩니다. 즉, 컴포넌트가 Context에 강하게 결합(Coupling)되어 다른 프로젝트나 다른 위치에서 독립적으로 재사용하기가 어려워집니다.
- **불필요한 리렌더링 (성능 이슈)**: Context의 value가 변경되면, 해당 Context를 구독(`useContext`)하고 있는 모든 하위 컴포넌트들이 리렌더링됩니다. 비록 컴포넌트 내부에서 Context의 특정 부분만 사용하더라도, 전체 value 객체가 바뀌면 리렌더링을 피하기 어렵습니다. (앞서 살펴본 것처럼 Context 분리 등의 추가적인 최적화 작업이 필요합니다.)

> [!TIP]
> **"그럼 그냥 모든 데이터를 담은 Context 하나로 전체 앱을 감싸면 편하지 않을까요?"**  
> 이론적으로는 가능하지만, 실무에서는 권장하지 않습니다. 앱 전체를 하나의 큰 Context로 감싸면, 아주 작은 상태 변화(예: 다크모드 전환)가 일어날 때마다 **앱 내의 거의 모든 컴포넌트가 리렌더링**될 수 있기 때문입니다. 또한, 전역 상태가 많아질수록 어떤 컴포넌트가 어떤 데이터를 쓰는지 추적하기가 매우 힘들어져 유지보수가 어려워집니다. 따라서 데이터의 성격이나 사용 범위에 따라 적절히 Context를 쪼개거나, 컴포넌트 합성을 활용하는 것이 좋습니다.

### 2. 해결 방법: 컴포넌트 합성 (Component Composition)
컴포넌트 합성이란 여러 개의 컴포넌트를 합쳐서 새로운 컴포넌트를 만드는 기법입니다. 쉽게 말해, **"자식 컴포넌트를 구성하는 책임을 부모가 아닌, 그 컴포넌트를 사용하는 곳(최상위)으로 넘기는 방식"**입니다.

이를 통해 중간 컴포넌트가 하위 컴포넌트에게 데이터를 전달하는 '다리 역할'을 할 필요가 없게 되어 의존성이 낮아집니다.

#### 1) 기존 방식 (의존성이 높은 구조)
중간 컴포넌트인 `Layout`이 특정 컴포넌트를 직접 렌더링하고 있을 때입니다. 데이터를 전달하기 위해 중간 컴포넌트를 거쳐야만 합니다.
...
const Layout = ({ user }) => {
  // Layout은 user 데이터가 필요 없지만, Nav에 전달하기 위해 받아야 함 (Props Drilling)
  return (
...
const Nav = ({ user }) => {
  return <nav>안녕하세요, {user.name}님!</nav>;
};
```

#### 2) 컴포넌트 합성 방식 (의존성을 제거한 구조)
`Layout`에서 특정 컴포넌트를 직접 렌더링하지 않고, `children`이라는 구멍을 뚫어놓는 방식입니다.
...
const App = () => {
  ...
  return (
    <Layout>
      {/* Layout의 자식으로 Nav를 직접 넣어줌 */}
      {/* 이제 App이 Nav에게 직접 데이터를 줄 수 있음! */}
      <Nav user={user} /> 
    </Layout>
  );
};
...
const Layout = ({ children }) => {
  // 이제 Layout은 user가 누구인지, Nav가 내부에 있는지 몰라도 됨!
  // 단지 주입받은 children을 원하는 자리에 배치만 함 (제어권 위임)
  return (
    <div className="layout">
      <Header />
      {children} 
...
#### 컴포넌트 합성의 장점
- **Props Drilling 방지**: 데이터를 필요로 하지 않는 중간 컴포넌트를 거치지 않고 자식에게 직접 데이터를 줄 수 있습니다.
- **재사용성 향상**: `Layout` 컴포넌트는 내부에 `Nav`뿐만 아니라 `Sidebar`, `Footer` 등 어떤 것이든 담을 수 있는 범용적인 상자가 됩니다.
- **결합도 낮추기**: 컴포넌트끼리 서로의 존재를 덜 알게 되어, 코드를 수정하거나 기능을 추가하기가 훨씬 쉬워집니다.
